...
- [x] **A5b. RAW develop nodes**: `highlight_reconstruction` (smoothstep desaturation toward the channel mean as max(r,g,b) approaches clip — magenta skies go white) + `shadow_lift` (multiplicative hyperbolic gain `1 + amount·range/(range+luma)` — true black fixed, hue preserved). Both pointwise + fusable, on all three backends (CPU / desktop GL / GLES, math character-identical), covered by the parity suite and semantic CPU tests; added to the editor palette.
...
**Phase A complete.** Parked follow-ups: gamut-compression node (replaces srgb_output's hard clamp, unlocks fusing it).
...
- [x] **B6. Scopes**: `ScopeRenderer` (gles-renderer) — GLES 3.1 compute shader, atomic-add luma+RGB histograms (256 bins) + 128×64 waveform into one SSBO, stride-2 sampling (¼ the pixels), sync readback (ponytail: fence + double-buffer if it shows in traces); self-disables on ES 3.0-only contexts. Zebra (clip stripes ≥0.98) + focus peaking (luma-Laplacian red overlay) as blit-time-only overlays — the returned final texture stays clean so scopes never bin overlay pixels. `ScopeGate` (pure, 4 tests): every 4th frame at FULL/REDUCED_GRAIN, every 8th at NO_GRAIN, off once spatial passes degrade — scope cost lands inside the PerfMonitor-measured frame so the ladder throttles it (§11 "drop grain quality → drop scope rate"). UI: Scope chip cycles Off→Hist→Wave, Zebra/Peak chips, Compose Canvas overlay (RGB+luma histogram bars, log-scaled waveform heat bitmap). On-device exercise pending a phone. Deferred: vectorscope + false color (§11 lists them; add with the editor's grading tools when there's a use), pre-film-sim peaking tap.
- [~] **B7. Device profile v1** — full pipeline built, awaiting the calibration shoot on the reference device:
  - `tooling/profile-calibrator` (new module): `DeviceProfile` schema + versioned JSON codec (black/white levels per ISO with interpolation, dual-illuminant matrices interpolated in mired à la DNG, noise ladder, per-lens vignette); `ColorMatrixSolver` (weighted least squares camera-RGB→XYZ, IRLS reweighted by ΔE76 — `ponytail:` swap for ΔE₀₀ if real charts demand it); shared `RadialPolyFitter` (vignette + Brown-Conrady distortion); `NoiseFitter` (variance = scale·signal + offset, degrades to offset-only on darks); `Calibrator` report→profile pipeline + CLI main; ColorChecker reference from the nominal sRGB rendition. 8 tests, all solvers verified against synthetic ground truth.
  - camera-core: `CalibrationStats` (pure Bayer stats — CFA-aware means/variances, 24-patch chart grid, radial gain bins; 4 JVM tests); `CameraController.captureCalibrationRaw` (in-memory RAW_SENSOR via temporary OUTPUT_FORMAT_RAW use case); `DeviceProfiles.harvest` + `loadProfile` (§16 runtime fallback from Camera2 SENSOR_COLOR_TRANSFORM/black-level/illuminant metadata; calibrated profile on disk wins).
  - app: CAL mode cycles OFF→DARK→CHART☀→CHART💡→FLAT; shutter appends frame stats to `calibration_report.json` (external files dir) and drops the harvested fallback profile alongside.
  - **remaining (hardware in hand):** shoot dark-frame ISO ladder + ColorChecker under daylight/tungsten + flat fields on the device, `adb pull` the report, run the calibrator, push `deviceprofile.json` back.

**Phase B complete except the B7 calibration shoot.** Parked follow-ups: GPU export/preview EGL context, pre-film-sim peaking tap, vectorscope/false color, HEIF capture, layer blending/masks (M3), dot-grid distortion capture flow (fitter is ready), profile consumption in the develop path (needs DNG decode).
- [x] **B8. Editor**: `EditStack` (camera-core, pure, 5 tests) — linear layer stack ↔ chain ProcessGraph bijection (layer.enabled ↔ node.enabled, terminal `srgb_output` implicit), palette of 10 node types with descriptor defaults; **a stored version IS a recipe** — history rows hold RecipeCodec JSON (A1 wire format + migration chain), so edit → reopen → re-edit is lossless and every version is shareable. Room (`edit_sessions` unique-by-uri, `edit_versions` append-only: no update/delete queries exist). `EditorScreen`/`EditorViewModel`: photo picker (PickVisualMedia, no storage permission), debounced (250ms) CPU preview ≤1280px on the *fused* plan, add/remove/reorder/toggle layers, per-param sliders from ParamSpec ranges, stock chips, version strip (tap = restore, save = append). `TiledRenderer.renderStreaming` (image-engine) — callback-fed tiles so neither full float source nor output ever exists in memory (48MP would need ~1.5 GB); existing `render` now wraps it, bit-identical tests still gate both. `ExportWorker`: BitmapRegionDecoder tile fetch (sample-size capped at 6144px long edge), fused plan, EXIF rotation applied to output, JPEG or HEIF (HeifWriter, runtime-gated API 28+, manifest overrideLibrary), graph JSON via cache file (WorkManager 10KB Data cap). Deferred: GPU export/preview EGL context, layer blending/masks (M3), true >6144px streaming encoder. On-device exercise pending a phone.
